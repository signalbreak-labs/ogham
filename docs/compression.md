...
store. Set `ccr_store_path: Some(path)` to persist originals in SQLite instead
— that requires the `ccr-sqlite` feature (a configured path without it is a
...
`ogham::ccr::compute_key(bytes)` — a versioned, collision-resistant content
address of the form `b3:<32 hex>` (a 128-bit BLAKE3 prefix), not a security
mechanism. The `b3:` version tag lets the hash scheme evolve while ids stored
under an older scheme stay retrievable.

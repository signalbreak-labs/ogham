...
crates/
  ...
  legacy-compress/          ABANDONED STUB — delete in WP-0
  legacy-compress-core/     ABANDONED STUB — delete in WP-0
  legacy-compress-server/   ABANDONED STUB — delete in WP-0
DESIGN.md                   stale pointer file — fix in WP-0
DESIGN.md                   this document (the build plan and engineering record)
```

Workspace `members` lists only the three `context-compress*` crates. The `legacy-*`
...
1. Delete directories `crates/legacy-compress`, `crates/legacy-compress-core`,
   `crates/legacy-compress-server` (they are not workspace members; nothing imports them).
...
- `grep -r "legacy-compress" crates/ Cargo.toml` returns nothing.
